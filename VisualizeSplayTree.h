...
#ifndef VisualizeSplay_h
#define VisualizeSplay_h
...
display_rows get_row_display(SplayTree *);
void Draw(SplayTree *);
vector<string> row_formatter(const display_rows &rows_disp);
int max_depth(SplayNode *);
//==============================================================
...
int max_depth(SplayNode *ptr) {
  int left_depth = 0;
  int right_depth = 0;

  if (ptr->getLeft() != NULL)
    left_depth = max_depth(ptr->getLeft());

  if (ptr->getRight() != NULL)
    right_depth = max_depth(ptr->getRight());
...
display_rows get_row_display(SplayTree *ptr) {
  // start off by traversing the tree to
  // build a vector of vectors of Node pointers
  vector<SplayNode *> traversal_stack;
  vector<vector<SplayNode *>> rows;

  if (!ptr->getRoot())
    return display_rows();

  SplayNode *node = ptr->getRoot();
  int levels = max_depth(ptr->getRoot());
  rows.resize(levels);
  int depth = 0;
...
void Draw(SplayTree *ptr) {
  const int d = max_depth(ptr->getRoot());
